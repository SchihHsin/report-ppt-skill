...
- **叙事先于版式**：先明确受众决策、整份核心判断与章节承接；页面标题写发现或设计判断，正文区分证据、推论与待验证方案。
- **章节 Tab**：浅底内容页右上使用整份汇报共用的一组章节 Tab，当前章节深色高亮；不放孤立页码、年份或说明小字。封面、目录页不放 Tab，黑底设计点页沿用自身的顶部信息。
...
用户说“目录 / agenda / contents / 章节概览”时，从 `assets/template-library/index.html` 复制 `contents-directory`。它只建立阅读路径，不提前写研究结论：每张卡为「两位章节号 → 中文名 → 英文辅助名 → 1--2 个内容锚点 → 章节职责」。

- 3 / 4 / 5 章：固定单行横排；6 章：3×2；7 章及以上先合并章节，或拆为主目录与附录目录。
- 左上仅写「目录」，不加英文副标题；章节号使用 `Barlow Condensed` 300 字重。
- 目录中的章节名须和各章节页、页脚、总结与附录保持一致。
...
- 新增 `references/narrative-and-copy.md`：先建立受众决策、核心判断、章节问题与承接；标题必须是发现、判断或设计选择，副标题只补范围、原因或证据边界；明确证据、推论与方案的表达边界。方案页可按「问题 → 期望用户状态 → 策略 → 设计点 → 收益 → 验证/依赖」组织。
- 新增 `contents-directory` 目录页组件：3 / 4 / 5 章单行，6 章 3×2，7+ 章合并或拆主目录与附录；左上仅「目录」，章节编号为 `Barlow Condensed` 300 字重。
- 浅底内容页右上改为横向章节 Tab：每页保持同一组章节与顺序，仅当前章节以深色胶囊高亮；移除孤立的页码、年份与说明小字。Tab 文字使用 `--fs-sm`，保证快速扫读。

### 2026-07 — 文案与卡片细节完善

- 强化「标题 = 观点凝练」：标题可脱离版式独立成立，并且一页只表达一个主结论；交付检查同步覆盖。
- 卡片内标签固定为随文字收缩的 chip，不可拉成整行色条；普通正文仍使用 `--fs-body`，信息过多优先删减或拆页。
...
- 新增 `references/information-density.md`：先按阅读意图区分灰底分析 `D1--D4`、灰底分析视觉 `A1--A3`、黑底设计叙事 `B1--B3`，不再只按字数决定页面。
- `D1--D4` 明确为低密度主张、标准证据分析、结构化中高密度分析、查阅型高密度对照四档；默认从 D1 开始，密度增加时优先增加结构，不缩普通正文。
- `A1--A3` 规定截图、材料和关系图必须承担分析证据；`B1--B3` 规定大图、界面和机制图承担设计叙事，不与 D 系列混用。
