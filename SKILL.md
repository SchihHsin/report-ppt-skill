...
- **同层级信息卡按数量选样式**：仅用于无专属组件的并列同级内容（不是竞品、指标、画像、流程或时间轴）。**恰好 3 张**必须是白玻璃 / `--g-accent` / `--g-ink` 三层，依次承载事实、重点、结论，不能三张同色白卡；**超过 3 张**必须全部为透白玻璃卡，使用 `--c-*` 驱动同卡标签、Lucide 大图标和顶部横向淡光带，禁止实色满铺、顶部色描边和每张卡重复同一种颜色。4--5 张默认靛/蓝/青/绿/粉，紫只在策略/创新语义下替换一张。**无论三张、四张或更多**，标题统一预留两行高度；标签、标题和说明组成一个避开右下水印的居中阅读组，标题 `--fs-h1`、说明 `--fs-h3`，不能做成贴顶标签 + 底部小字。完整骨架见 `components.md §17` 与模板库。
