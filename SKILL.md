...
3. **逐页选件（关键，别跳）**：把用户每页内容**先过一遍 `components.md` 顶部的「选件路由表」**——用户极少说组件学名（「VOC 墙」「用户旅程」…），要靠**意图/大白话**命中（「这些吐槽」→VOC 墙、「各阶段体验」→用户旅程、「友商对比」→竞品对照…）。命中后**照抄该节精调骨架**，只改文案/配色；**没命中再考虑自定义版面**，绝不一上来就手搓。
