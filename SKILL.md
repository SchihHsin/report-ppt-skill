...
3. **逐页选件（关键，别跳）**：对每页内容先按下表路由，命中就去 `components.md` 对应节**照抄骨架 + 填充示例**，只改文案/配色；没命中才考虑自定义，**绝不一上来手搓**。
   ...
   | 介绍某个用户/他是谁/他的日常/角色特征/职责场景痛点/persona（单人深描） | **§6a 用户画像·形式一**（左栏+右侧多区） |
   | 几类用户有什么不同/角色对比/能力维度对比/雷达图/多角色对照 | **§6b 用户画像·形式二**（顶栏+雷达） |
...
- **⛔ 页面标题前只放 CANN logo，或什么都不放——绝不加装饰性 icon**（复用 skill 时最常翻车的一条）：head 的 `.brand` 槽默认放 CANN logo（`<img class="logo" src="CANNlogo.png">`，深底用 `cann-dark-logo.svg`）；用户给了别的真 logo 就换，都没有就把 `<img>` 删掉留空。**严禁**在标题/区块标题前顶 Lucide / 通用 SVG 图标。装饰 icon 仅限个别精调组件内部（如画像 `.pf-h2`），不是通用模式。
...
- `assets/deck-template.html` — 可用单文件 deck 起手（**7 个样例页全是真·精调件、覆盖三基调**：① 封面 ② 关键指标(多彩渐变胶囊·精调①) ③ 竞品对照(三卡+对策条) ④ 用户画像·形式一 ⑤ 甘特 roadmap(精调②) ⑥ 黑底章节封面(调色面板·持久记忆) ⑦ 黑底设计点；自带真实素材 `cover-bg.png`/`cann-dark-logo.svg`(深色封面用白版 logo)/`CANNlogo.png`(浅色页用深版 logo)/`persona.svg`/`cmp-*.jpg`）。**新材料从这里开始拷**，删改样例页 + 从 `components.md` 取更多组件。⚠️ 用户画像形式一/二是**两套方案二选一**，按输入择优（详见 components.md §6）
