...
- **颜色分两层用**：小面积语义元素（标签、Lucide 图标、线/点、卡片局部光带）用 `--c-*` 纯色；大面积强调面（突出卡、宽色带、光晕、面积填充）用同色相 `--g-*` 渐变。每页选 1 主色 + 至多 2 辅色，不能为了“色谱全”做彩虹墙；红/橙/黄只用于风险、提醒、进行中等语义，绿/薄荷用于机会和完成，`--g-ink` 统一用于深色大块。
