...
## 6c. 用户画像·形式三（左紫栏档案 + 右便利贴墙 / 看板式）

> **触发**：「工作坊/共创式的画像」「把一个角色的方方面面铺开」「便利贴/看板风格的 persona」「性格+动机+目标+痛点+问题+解法都要」—— **单人全貌速写**，纯 CSS 无外部依赖，比形式一更轻快、更"白板感"。

结构（外层 `.up` = `grid-template-columns:235px 1fr`）：
- **左栏 `.up-side`**（紫色渐变档案卡 `linear-gradient(160deg,#6366D8,#8268E2)`）：头像（横幅 `aspect-ratio:7/5`）+ 姓名 + 角色 + 信息字段网格（年龄/城市/岗位/司龄）+ 个人简介 + **特征标签便利贴**（白底小方贴）。
- **右栏 `.up-grid`**（`repeat(4,1fr)` × `1fr auto` 两行）：上排 4 格 = 性格（滑杆 `.up-pl`）/ 动机（进度条 `.up-mot`）/ 目标·痛点（彩色便利贴墙 `.up-wall`）/ 技术熟练度（圆点 `.up-dots`）+ 兴趣（灰胶囊 `.up-chips`）；下排 2 个通栏 = 问题 / 解法（便利贴墙）。
- 便利贴 `.up-note`：切角去圆角、上深下浅渐变（`linear-gradient(180deg,深,浅 22%)`）、轻微旋转错落，**同色系分 5 档**（黄y/红r/蓝b/绿g/紫p，每档带浅色变体 y2/r2…）。

### ⚠️ 等高 / 高屏三个坑（都踩过，实测定位）
1. **`.up` 别用 `flex:1` 填满整页** → 高屏（如 4K 1890px）会把整块拉到上千 px、便利贴被 `flex:1` 撑到离谱。用 **`flex:0 1 auto`（不长高、可压缩）+ 顶对齐**，按内容自适应。
2. **左右等高**靠 grid 默认 `align-items:stretch`（两列同高）；但右栏内部行**别用 `1.3fr .82fr`**——高屏会把下排也按比例注水、内容少就空一截、和左栏错位。用 **`grid-template-rows:1fr auto`**（上排吃满剩余、下排按内容贴合）。
3. **特征标签便利贴**做成 `.up-notes{flex:1;display:grid;grid-template-columns:1fr 1fr}` 的弹性 2×2 网格、`min-height:0` 可缩 —— 它是左栏唯一弹性区，用来吸收和右栏的高度差。**别给便利贴设固定 min-height**，否则矮屏缩不动、紫栏溢出比白卡高。
> 调这页"对不齐/太高"时**别靠 CSS 推理猜，用无头 Chrome 截图 + getBoundingClientRect 量像素**（见样板调试史）。

## 三种画像形式怎么选（路由命中后二次确认）

| 形式 | 结构 | **什么时候用** | 依赖 |
|---|---|---|---|
| **§6a 形式一** | 左人物栏 + 右四区（conic 环 / 协同 flow / 场景 mockup / 痛点） | 要**深入刻画一个核心角色**：职责、典型业务场景、痛点全貌，信息密度高、最"正式" | 纯 CSS |
| **§6b 形式二** | 顶栏 + ECharts 雷达 + 三栏 | 要**多角色对比 / 能力维度量化**（谁强谁弱、对某事的敏感度），靠雷达图说话 | **ECharts** |
| **§6c 形式三** | 左紫栏档案 + 右便利贴墙（看板） | 要**轻快地铺开一个角色的方方面面**（性格/动机/目标/痛点/问题/解法），工作坊/共创/白板风，无需图表库 | 纯 CSS |

一句话：**讲透一个人=形式一；比较几个人=形式二；白板式速写一个人=形式三。** 形式一与形式三都讲单人，区别在"正式深描 vs 便利贴速写"。
...
.dh-lead{font-size:clamp(17px,1.3vw,22px);color:var(--ink-2);line-height:1.7;text-align:center;max-width:1000px;margin:0 auto}   /* 总体说明，宽度=卡片行宽；一句话别太长(会折两行) */
...
## 12. 生态干系人地图（同心圈层 stakeholder / ecosystem map）

> **命中**：干系人/利益相关者/生态/stakeholder map/ecosystem/「以 X 为中心」的关系图。整张图 = **一个内联 SVG**（`viewBox 0 0 1280 720`，`preserveAspectRatio` 控居中），手画最忠实——别用 ECharts 力导向（出不来这种同心圈层+卫星+关系箭头的特定排版）。
>
> **结构**：中心人物（核心用户）→ 同心三圆（工作台→团队→生态）→ 外围散布**卫星圆**（各类干系人/服务）→ 一侧孤立圆（局外/本地）+ 叠加**价值圆**（双方价值观）→ 角落关键词区（痛点/沟通）→ 从中心**辐射的带标签曲线箭头**（关系/流向）→ 底部**配色图例**。
>
> **关键做法/坑**：
> - **配色分区**：卫星按类别上色（accent/teal/purple/amber/red/slate），用「玻璃 tint」——`fill=低透明同色线性渐变 + stroke=同色由浓到淡渐变`（仿透白卡，**别用实心色块**，重）；defs 里每个 zone 两条 `linearGradient`（`ff-*` 填充 / `bf-*` 描边）。文字用深 ink（浅 tint 上）。
> - **人物**用标准 Material person 图标路径（`M12 12c2.21 0…`）包在 `<g transform="translate scale">` 里，按角色上色——**别手搓头+身**（头身易分离/比例怪，反复踩）。
> - **箭头**从中心**按角度排序辐射**（同一原点的射线天然不交叉）；曲线 label 放各自中点、偏到线外，**绕开人物与文字清单**。
> - **同心圆**用冷灰渐深或半透白叠加；最外圈若和底部波浪纹理糊在一起 → 调成更深/偏冷的灰，或把 `.slide` 波浪透明度调淡。
> - 圆上文字用 `.up-note` 之外的 `text`；圆心用 `text-anchor:middle` + 两行 `<tspan>`。
> - **可选「左右版」**：左半 HTML 文案卡（结论 h2 + 正文 + 痛点/沟通玻璃卡 + 图例 chips），右半**透白玻璃卡**里放这张 SVG（`width/height:100%` + `xMidYMid meet` 自动居中）；想让图更大/居中 → **收紧 viewBox 到内容包围盒 + 均匀小边距**。

```css
.smap{position:relative;flex:1;min-height:0;display:flex}
.smap svg{width:100%;height:100%;display:block}
.sm-c1{fill:#E8EAEE}.sm-c2{fill:#DEE1E6}.sm-c3{fill:#D4D8DF}   /* 同心圆：冷灰、等步长渐深；或最外层 fill:#fff;fill-opacity:.32 半透白 */
.sm-z-dev{fill:url(#ff-dev);stroke:url(#bf-dev);stroke-width:1.4}   /* 玻璃 tint 卫星：accent；其余 hw=teal #3BA9E0 / comm=purple #7C3AED / ops=amber #D9850B / risk=red #E5484D / plat=slate #5A6473 */
.sm-onlbl{fill:#2c323b;font-weight:600}                            /* 圆上深色标签 */
.sm-arrow{stroke:#8b929d;stroke-width:1.1;fill:none;opacity:.7}    /* 关系箭头（细、淡）*/
.sm-arrlbl{fill:#868d98;font-style:italic}                         /* 箭头斜体标签 */
.sm-fig-acc{fill:#5B5BD6}.sm-fig-warm{fill:#D9850B}.sm-fig-cool{fill:#3BA9E0}  /* 人物按角色上色 */
```
```html
<defs>
  <marker id="sm-ah" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M0 0 L10 5 L0 10 z" fill="#7e8694"/></marker>
  <!-- 每个 zone 两条渐变：填充 ff-* / 描边 bf-* -->
  <linearGradient id="ff-dev" x1="0" y1="0" x2="0.35" y2="1"><stop offset="0" stop-color="#5B5BD6" stop-opacity=".20"/><stop offset="1" stop-color="#5B5BD6" stop-opacity=".11"/></linearGradient>
  <linearGradient id="bf-dev" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#5B5BD6" stop-opacity=".3"/><stop offset="1" stop-color="#5B5BD6" stop-opacity=".08"/></linearGradient>
  <!-- …ff-hw/bf-hw/ff-comm/… 同模式 -->
</defs>
<circle class="sm-c1" cx="648" cy="353" r="295"/><circle class="sm-c2" cx="618" cy="383" r="205"/><circle class="sm-c3" cx="548" cy="403" r="120"/>
<g><circle class="sm-z-hw" cx="942" cy="98" r="50"/><text class="sm-onlbl" text-anchor="middle" font-size="13"><tspan x="942" y="93">芯片</tspan><tspan x="942" dy="16">厂商</tspan></text></g>
<!-- …其余卫星圆… -->
<path class="sm-arrow" marker-end="url(#sm-ah)" d="M590,394 Q812,318 1088,350"/><text class="sm-arrlbl" font-size="12" text-anchor="middle" x="820" y="338">文档查阅</text>
<g class="sm-fig-acc" transform="translate(512,367) scale(3)"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></g>
```
> 图例 chips（HTML，叠在 SVG 外）：`.sm2-legend .ki{display:flex;align-items:center;gap:.35em;font-size:clamp(10px,.68vw,12px);color:var(--ink-2)}` + `.dot{width:8px;height:8px;border-radius:50%}`（dot 用 zone 实心色）。

## 13. 2×2 机会 / 优先级矩阵（价值 × 成本，便利贴版 + 左右布局）

> **命中**：优先级/机会矩阵/价值×成本/影响×成本/2×2/四象限/impact-effort/quick wins/先做什么。
>
> **轴**：Y=用户价值/影响（↑），X=实现成本/难度（→）。四象限：**快赢**(高价值低成本·先做) / **重点投入**(高价值高成本·分阶段) / **看情况**(低价值低成本) / **暂缓**(低价值高成本)。标题/正文写**结论**（点名哪些先做），别写"把痛点散布到四象限"这种动作描述。
>
> **推荐做法**：**bento 四块**（4 个独立圆角 tint tile、等距 gap，**不要用细线分隔单面板**）；条目用**便利贴 `.up-note`**（复用画像便利贴：黄/红/蓝/绿折角贴）在各 tile 内**按价值×成本相对散布**（不是每个居中）。
> - **坐标轴 = 五边形「燕尾」箭头**（整体一根渐变条，末端 `clip-path` 收成尖角）——不是末端单独冒出的开口箭头，也不是比线宽的实心三角。渐变「透明→浅灰 `#aab0ba`」、尖端收 ~7px。
> - **左右布局**：左半文案（kicker?/结论 h2 + 正文 p + 彩色胶囊图例 chips，h2≤页眉标题、加 `margin-bottom` 与正文拉开），右半**透白玻璃卡**装矩阵；正文 `p b` 加粗用「荧光笔」底色 `background:linear-gradient(transparent 58%,rgba(91,91,214,.18) 0)`。

```css
.m4{flex:1;min-height:0;display:grid;grid-template-columns:0.64fr 1.36fr;gap:2vw;align-items:center}   /* 左文案 + 右矩阵 */
.m4-grid{display:grid;grid-template-columns:1fr 1fr;grid-template-rows:1fr 1fr;gap:12px}                /* bento 四块、等距 */
.m4-cell{position:relative;border-radius:14px;overflow:hidden}
.m4-cell.tl{background:rgba(35,207,160,.13)}.m4-cell.tr{background:rgba(91,91,214,.12)}.m4-cell.bl{background:rgba(245,158,11,.13)}.m4-cell.br{background:rgba(229,72,77,.11)}
.m4-cell .ct{position:absolute;z-index:2;font-weight:700;font-size:max(12px,.92vw)}   /* 象限标题在外角 */
.m4-cell .up-note{position:absolute}                                                   /* 便利贴绝对定位、各自 left/top% + rotate */
/* 燕尾五边形坐标轴（X 朝右）：一根渐变条末端收尖 */
.m4-xaxis .pent{height:13px;width:100%;background:linear-gradient(to right,rgba(170,176,186,0),#aab0ba);clip-path:polygon(0 0,calc(100% - 7px) 0,100% 50%,calc(100% - 7px) 100%,0 100%)}
.m4-yaxis .pent{width:13px;height:100%;background:linear-gradient(to top,rgba(170,176,186,0),#aab0ba);clip-path:polygon(0 100%,0 7px,50% 0,100% 7px,100% 100%)}
/* 右半透白玻璃卡 */
.m4-right{background:rgba(255,255,255,.34);backdrop-filter:blur(14px) saturate(1.4);border:1px solid rgba(255,255,255,.6);border-radius:16px;box-shadow:0 12px 30px -16px rgba(30,40,70,.22);padding:3.4vh 2vw}
```
```html
<div class="m4-cell tl"><div class="ct q-win">快赢<small>高价值 · 低成本</small></div>
  <div class="up-note g" style="left:40%;top:48%;transform:translate(-50%,-50%) rotate(-2deg)">环境一键就绪</div>
  <div class="up-note g2" style="left:64%;top:26%;transform:translate(-50%,-50%) rotate(2deg)">智能报错诊断</div></div>
```
...
> **命中**：灰底页**只要是「图 / 图表 / 表格 / mockup + 一段文字」的形态，就用这个**——这是由内容形态直接触发的范式，不是"别的没匹配才轮到它"。例外：内容本身是旅程 / VOC / 能力矩阵 / 架构 / 2×2 等**已有专属版式**的，走各自的件（它们也是图+文，但版式更贴合）。
>
